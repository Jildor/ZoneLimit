...
UPDATE `creature_template` SET `AIName`='SmartAI',`minlevel`=70,`maxlevel`=71,`exp`=1,`flags_extra`=0 WHERE `entry`=@ENTRY;
...
SET @ENTRY := 26676;
-- Lo pongo como en TDB
UPDATE `creature_template` SET `AIName`='SmartAI',`faction_A`=2018,`faction_H`=2018,`mechanic_immune_mask`=8388624 WHERE `entry`=@ENTRY;
DELETE FROM `smart_scripts` WHERE `entryorguid` IN (@ENTRY,@ENTRY*100);
INSERT INTO `smart_scripts` (`entryorguid`,`source_type`,`id`,`link`,`event_type`,`event_phase_mask`,`event_chance`,`event_flags`,`event_param1`,`event_param2`,`event_param3`,`event_param4`,`action_type`,`action_param1`,`action_param2`,`action_param3`,`action_param4`,`action_param5`,`action_param6`,`target_type`,`target_param1`,`target_param2`,`target_param3`,`target_x`,`target_y`,`target_z`,`target_o`,`comment`) VALUES
(@ENTRY,0,0,0,54,0,100,1,0,0,0,0,49,0,0,0,0,0,0,23,0,0,0,0,0,0,0,"Anub'ar Invader - Just Summoned - Attack Summoner");
-- Borro spawn innecesario
DELETE FROM `creature` WHERE `id`=@ENTRY;
-- Borro template_addon
