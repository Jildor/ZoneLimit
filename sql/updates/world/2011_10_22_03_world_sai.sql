...
UPDATE `creature_template` SET `unit_flags`=33536,`flags_extra`=0,`AIName`='SmartAI' WHERE `entry` IN (@ENTRY_HATHOREL,@ENTRY_TYBALIN);
-- Le añado template_addon como en TDB
DELETE FROM `creature_template_addon` WHERE `entry`=@ENTRY_TYBALIN;
INSERT INTO `creature_template_addon` (`entry`,`path_id`,`mount`,`bytes1`,`bytes2`,`emote`,`auras`) VALUES
(@ENTRY_TYBALIN,0,0,0,1,0,NULL);
-- Las pongo como en TDB
UPDATE `quest_template` SET `SrcItemId`=0,`SrcItemCount`=0,`SrcSpell`=0,`RewSpellCast`=71098,`OfferRewardText`="Excellent work. Now that this is in our hands, we must get it to Caladis Brightspear as soon as possible. We can't risk the book falling back into Sunreaver hands.$BI'll ensure that Shandy Glossgleam gets his tabard back without incident, along with a healthy reward for his help",`RequestItemsText`="Have you recovered that book?" WHERE `entry`=@QUEST_A;
UPDATE `quest_template` SET `SrcItemId`=0,`SrcItemCount`=0,`SrcSpell`=0,`RewSpellCast`=71098,`OfferRewardText`="Excellent work. Now that this is in our hands, we must get it to Myralion Sunblaze as soon as possible. We can't risk the Silver Covenant taking the book from us again.$BI'll get that tabard back to Shandy and see that he's well compensated for his assistance. Once the book is in Myralion's hands, the Silver Covenant won't be a threat anymore.",`RequestItemsText`="Have you recovered that book?" WHERE `entry`=@QUEST_H;
...
-- Le quito gossip_scripts y string sobrantes
DELETE FROM `gossip_scripts` WHERE `id` IN (@GOSSIP_A,@GOSSIP_H);
DELETE FROM `db_script_string` WHERE `entry` IN (2000000432,2000000433,2000000434,2000000435,2000000436);
