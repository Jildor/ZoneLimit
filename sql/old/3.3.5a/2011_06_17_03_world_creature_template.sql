-- Editado por Gildor
-- Les quito los AIScripts
UPDATE `creature_template` SET `AIName`='' WHERE `entry`=33515; -- Freya
UPDATE `creature_template` SET `AIName`='',`ScriptName`='npc_feral_defender' WHERE `entry`=34035; -- Feral Defender
UPDATE `creature_template` SET `AIName`='',`ScriptName`='npc_sanctum_sentry' WHERE `entry`=34014; -- Sanctum Sentry
UPDATE `creature_template` SET `ScriptName`='npc_auriaya_seeping_trigger',`unit_flags`=33554692,`flags_extra`=130 WHERE `entry` =34098; -- Seeping Trigger
UPDATE `creature_template` SET `ScriptName`='',`unit_flags`=33554692,`flags_extra`=128 WHERE `entry` =34096; -- Feral Defender Trigger